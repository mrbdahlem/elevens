...
public class CardGameGUI extends JFrame implements ActionListener {
    ...
    private ImageIcon[][] cardsSelected;
    ...
    public void repaint() {
        for (int k = 0; k < board.size(); k++) {
            ImageIcon icon = cardImage(board.cardAt(k), selections[k]);
    ...
    /**
     * Load the cards and break apart into individual images
     */
    private void initCards() {       
        BufferedImage allCards = null;
        
        // Try to load card images from the classpath (jar file)
        try {
            InputStream cardStream = getClass().getResourceAsStream("cards.gif"); 
            allCards = ImageIO.read(cardStream);
            
        } catch (IOException ex) {
            // If that fails, try the current directory                
            try{
                File cardFile = Paths.get("").resolve("cards.gif").toFile();   
                allCards = ImageIO.read(cardFile);
            }
            catch (IOException e) {
                JOptionPane.showMessageDialog(null, "Could not load cards.gif", "Error", JOptionPane.ERROR_MESSAGE);
                System.exit(1);
            }
        }
        
        cards = new ImageIcon[4][13];
        cardsSelected = new ImageIcon[4][13];
        
        // Break the cards into
        for (int suit = 0; suit < 4; suit++) {
            for (int rank = 0; rank < 13; rank++) {
                cards[suit][rank] = new ImageIcon(allCards.getSubimage(
                        rank * CARD_WIDTH,
                        suit * 2 * CARD_HEIGHT,
                        CARD_WIDTH,
                        CARD_HEIGHT));
                cardsSelected[suit][rank] = new ImageIcon(allCards.getSubimage(rank * CARD_WIDTH,
                        ((suit * 2) + 1) * CARD_HEIGHT,
                        CARD_WIDTH,
                        CARD_HEIGHT));
            }            
        }
        
        cardBack = new ImageIcon(allCards.getSubimage(0, CARD_HEIGHT * 8, CARD_WIDTH, CARD_HEIGHT));
    }

    /**
     * Get the paintable image for a particular card
     * @param   card the card to draw
     * @param   selected true if the card has been selected by the player
     * @return  the image representation for the card
     */
    private ImageIcon cardImage(Card card, boolean selected) {
        ...
        int rankNum = ranks.indexOf(rank);
        
        if (selected) {
            return cardsSelected[suitNum][rankNum];
        }
        else {
            return cards[suitNum][rankNum];
        }
    }
